...
import tw.elliot.domain.core.Auditable;
import tw.elliot.domain.core.Identifiable;
import tw.elliot.domain.support.Condition;
import tw.elliot.domain.support.LikeMode;
import tw.elliot.domain.support.Page;
import tw.elliot.util.ThreadLocalHolder;
...
public abstract class AbstractBaseDao<T extends Identifiable<Oid>, Oid extends Serializable> {
	...
	protected void preCreate(T entity) {
		if (entity instanceof Auditable) {
			...
			audit.setCreatedAccount(ThreadLocalHolder.getOperator().getName());
	...
	protected void preUpdate(T entity) {
		if (entity instanceof Auditable) {
			Auditable audit = (Auditable) entity;
			audit.setModifiedAccount(ThreadLocalHolder.getOperator().getName());
